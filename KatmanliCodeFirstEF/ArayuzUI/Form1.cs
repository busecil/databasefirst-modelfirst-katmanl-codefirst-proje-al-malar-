...
namespace ArayuzUI
{
    public partial class Form1 : Form
    {
        ...
        void arayuzListesiYildizla(List<Ogrenci> ogrList)
        {
            foreach (var ogr in ogrList)
            {
                ogr.KimlikNo = ogr.Kisalt(ogr.KimlikNo);
                ogr.OgrenciNo = ogr.Kisalt(ogr.OgrenciNo);
                ogr.Ad = ogr.Kisalt(ogr.Ad);
                ogr.Soyad = ogr.Kisalt(ogr.Soyad);
            }
        }
        ...
        {
            OgrenciDersFormu odf = new OgrenciDersFormu();
            odf.ShowDialog();
        }
    }
}
